...
func (r *teacherRepo) Create(ctx context.Context, teacher *models.User) error {
	err := r.db.QueryRowContext(ctx, createTeacher, &teacher.Firstname, &teacher.Lastname, &teacher.Password, &teacher.Role, &teacher.CreatedAt, &teacher.UpdateAt, &teacher.Verified).Scan(&teacher.ID)
	if err != nil {
		return err
	}
	return nil
